...
purchase_data1403to1406=pd.DataFrame(prtwd1403to1406['total_purchase_amt']);
purchase_data1403to1406_diff= purchase_data1403to1406.diff(1)
fig = plt.figure(figsize=(12,8))
ax1=fig.add_subplot(211)
fig = sm.graphics.tsa.plot_acf(purchase_data1403to1406_diff,title='total_purchase_amt acf',lags=40,ax=ax1)
ax2 = fig.add_subplot(212)
fig = sm.graphics.tsa.plot_pacf(purchase_data1403to1406_diff,title='total_purchase_amt pacf',lags=40,ax=ax2)

redeem_data1403to1406=pd.DataFrame(prtwd1403to1406['total_redeem_amt']);
redeem_data1403to1406_diff=redeem_data1403to1406.diff(1)
fig = plt.figure(figsize=(12,8))
ax1=fig.add_subplot(211)
fig = sm.graphics.tsa.plot_acf(redeem_data1403to1406_diff,lags=40,title='total_redeem_amt acf',ax=ax1)
ax2 = fig.add_subplot(212)
fig = sm.graphics.tsa.plot_pacf(redeem_data1403to1406_diff,lags=40,title='total_redeem_amt pacf',ax=ax2)


redeem_data1403to1406=redeem_data1403to1406.astype('float64');
purchase_data1403to1406=purchase_data1403to1406.astype('float64');

redeem_data1403to1406_diff=redeem_data1403to1406_diff.astype('float64');
purchase_data1403to1406_diff=purchase_data1403to1406_diff.astype('float64');
redeem_data1403to1406_diff.dropna(axis=0,how='any',inplace=True);
purchase_data1403to1406_diff.dropna(axis=0,how='any',inplace=True);
print('begin ARMA');
...
for p in range(0,15):
    for q in range(0,15):
        if(p!=q):
            try:
                arma_model = sm.tsa.ARIMA(redeem_data1403to1406,[p,1,q]).fit();
                print('redeem ARMA(%d,%d)AIC:%f BIC:%f HQIC:%f'%(p,q,arma_model.aic,arma_model.bic,arma_model.hqic));
            except:
                continue 
...
for p in range(0,15):
    for q in range(0,8):
        if(p!=q):
            try:            
                arma_model = sm.tsa.ARIMA(purchase_data1403to1406,[p,1,q]).fit();
                print('purchase ARMA(%d,%d)AIC:%f BIC:%f HQIC:%f'%(p,q,arma_model.aic,arma_model.bic,arma_model.hqic));
            except:
                continue
'''
...
warnings.filterwarnings("ignore")
redeem_arma_model = sm.tsa.ARIMA(redeem_data1403to1406.astype('float64'),[4,1,3]).fit();
purchase_arma_model = sm.tsa.ARIMA(purchase_data1403to1406.astype('float64'),[2,1,4]).fit();
...
redeem_predict_diff= redeem_arma_model.predict('2014-07-01', '2014-07-31', dynamic=False);
redeem_predict =redeem_predict_diff.cumsum().add(redeem_data1403to1406.ix['2014-06-30','total_redeem_amt']);

fig, ax = plt.subplots(figsize=(12, 8));
redeem_predict.plot(ax=ax,label='predict');
prtwd1407['total_redeem_amt'].plot(ax=ax,label='test');
plt.legend();
plt.title('july\'s total_redeem_amt predict')
plt.draw();
#计算误差
#redeem_error=np.divide(np.abs(np.array(prtwd1407['total_redeem_amt'])-np.array(redeem_predict).T),np.array(prtwd1407['total_redeem_amt']));
#print(



purchase_predict_diff= purchase_arma_model.predict('2014-07-01', '2014-07-31', dynamic=False);
purchase_predict =purchase_predict_diff.cumsum().add(purchase_data1403to1406.ix['2014-06-30','total_purchase_amt']);
                                                        
fig, ax = plt.subplots(figsize=(12, 8));
purchase_predict.plot(ax=ax,label='predict');
prtwd1407['total_purchase_amt'].plot(ax=ax,label='test');
plt.legend();
plt.title('july\'s total_purchase_amt predict')
plt.draw();
#redeem_error=np.divide(np.abs(np.array(prtwd1407['total_purchase_amt'])-np.array(purchase_predict).T),np.array(prtwd1407['total_purchase_amt']));
#print(redeem_error);
